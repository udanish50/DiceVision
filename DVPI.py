...
# Displaying all images for each category
...
def laplacian_kernel(x, y, sigma=1.0):
    l1_dist = np.sum(np.abs(x[:, None] - y[None, :]), axis=2)
    return np.exp(-l1_dist / sigma)
    
def exponential_kernel(x, y, sigma=1.0):
    l2_dist = np.sqrt(np.sum((x[:, None] - y[None, :])**2, axis=2))
    return np.exp(-l2_dist / sigma)
...
# Compute  scores
kid_stable_diffs = DICEVISION(originals, stable_diffs, inception_model, 4)
kid_dells = DICEVISION(originals, dells2, inception_model,4)
kid_glides = DICEVISION(originals, glides, inception_model,4)
kid_dells2 = DICEVISION(originals, dells3, inception_model,4)
